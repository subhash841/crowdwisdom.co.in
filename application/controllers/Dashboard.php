...
class Dashboard extends Base_Controller {
    ...
    function updateUserForecast() {
        ...
        if ( $is_result_out == "1" ) {
            echo json_encode( array ( "status" => FALSE, "message" => "Forecast programe has stoped now.", "redirect_url" => "" ) );
            return false;
        }
        if ( $_SESSION[ 'data' ][ 'alise' ] == "" ) {
            echo json_encode( array ( "status" => FALSE, "message" => "Please enter your alise first.", "redirect_url" => "Profile" ) );
            return false;
        }
        if ( $_SESSION[ 'data' ][ 'location' ] == "" ) {
            echo json_encode( array ( "status" => FALSE, "message" => "Please complete your profile first.", "redirect_url" => "Profile" ) );
            return false;
        }
