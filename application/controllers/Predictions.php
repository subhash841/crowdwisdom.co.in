...
class Predictions extends CI_Controller {
    ...
    function __construct() {
        ...
        if (!empty($sessiondata)) {
            ...
            $this->user_email = $sessiondata['email'];
        } else {
            $this->user_id = 0;
            $this->user_alias = "";
    ...
    function details($id) {
        ...
        $inputs['id'] = $id;
        $inputs['user_id'] = $this->user_id;
        $inputs['user_email'] = $this->user_email;

        $data = $this->Predictions_Mod->get_prediction_detail($inputs);

        $topic_ids = $data['data'][0]['topic_associated'];
        if (!empty($topic_ids)) {

            $category = $this->Predictions_Mod->get_category(array_column($topic_ids, 'topic_id'));
            if ($category['category_count'] > 0) {
                $data['election_category_exist'] = 'true';
            }
        }
    ...
    function answered_predictions() {
        ...
        $data = $this->Predictions_Mod->get_answered_prediction($inputs);
        echo json_encode($data);
    }

    function raise_prediction($id = 0) {
        $sessiondata = $this->session->userdata('data');

        if (!empty($sessiondata)) {
            $data = array();
            $header_data['page_title'] = "Raise Predictions";
            $header_data['page_img'] = base_url("images/logo/prediction_share_logo.png");
            $header_data['uid'] = $this->user_id;
            $header_data['silver_points'] = $this->silver_points;
            $header_data['id'] = $id;
            ...
            $this->load->view('bootstrap_header', $header_data);
            $this->load->view('raise_prediction', $data);
            $this->load->view('bootstrap_footer');
        } else {
    ...
    function create_update_prediction() {
        ...
        $this->form_validation->set_rules('emails', 'Email', 'trim|required');
        ...
        if ($this->form_validation->run() === FALSE) {
            $errors = array(
                ...
                "emails" => form_error('emails'),
                    // "uploaded_filename" => form_error( 'uploaded_filename' )
            );
            echo json_encode(array("status" => FALSE, "message" => "error", "data" => $errors));
        } else {
            $inputs = $this->input->post();
            $id = $inputs['predictionid'];
            $inputs['user_id'] = $this->user_id;
            $inputs['user_email'] = $this->user_email;

            $this->Predictions_Mod->add_update_prediction($inputs);
            $message = ($id == "0") ? "Private prediction created successfully" : "Private prediction update successfully";

            echo json_encode(array("status" => true, "message" => $message, "data" => []));
        }
    }
    ...
    function vote_action() {
        ...
        if (!empty($sessiondata)) {
            ...
            $isvote = $this->Predictions_Mod->vote_action_mod($input, $user_id);
            $id = $input['id'];
    ...
    function add_comment() {
        ...
        if ($user_id > 0) {
            ...
            if (!empty($user_id) && !empty($pollid) && !empty($poll_comment)) {
                $id = $this->Predictions_Mod->add_comment_mod($inputs);
                $comment_data = $this->Predictions_Mod->get_comment_by_id($id);
    ...
    function add_comment_reply() {
        ...
        if ($user_id > 0) {
            ...
            if (!empty($user_id) && !empty($pollid) && !empty($comment_id) && !empty($poll_comment_reply)) {
                $id = $this->Predictions_Mod->add_comment_reply_mod($inputs);
                $reply_data = $this->Predictions_Mod->get_comment_reply_by_id($id);
    ...
    function delete_comment() {
        ...
        $data = $this->Predictions_Mod->delete_comment_mod($this->user_id, $id, $comment_id);
        $this->apiresponse->sendjson($data);
    }
    ...
    function deactive_poll() {
        ...
        if (!empty($pollid)) {
            $this->apiresponse->sendjson($this->Predictions_Mod->make_deactive_poll($pollid));
        } else {
    ...
    function trend_detail() {
        $inputs = $this->input->post();
        $result = $this->Predictions_Mod->get_trend_result($inputs);
        $this->apiresponse->sendjson($result);
    }
