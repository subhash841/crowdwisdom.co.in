<?php
if ($data[0]['is_public'] == "2") {
    $emails = explode(",", $data[0]['email_ids']);
    $filtered_mails = array();
    foreach ($emails as $mails) {
        $filtered_mails[] = trim($mails);
    }
    $sessiondata = $this->session->userdata('data');
    if (!empty($sessiondata)) {
        if (!in_array($sessiondata['email'], $filtered_mails) && $data[0]['user_id'] !== $uid) {
            redirect("Predictions");
        }
    } else {
        $pid = str_replace("/Predictions/details/", "", $_SERVER['REQUEST_URI']);
        redirect("Login?section=predictiondetail&vid=$pid");
    }
}
$prediction_id = $data[0]['id'];
$user_location = $_SESSION['data']['location'];
$user_alise = $_SESSION['data']['alise'];

$allow_to_for_election;
if (!empty($user_location) && !empty($user_alise)) {
    $allow_to_for_election = 'true';
} else {
    $allow_to_for_election = 'false';
}
?>
<div class="container mb-5 data-container poll">
    <div class="row mt-3">
        <div class='col-md-9 voice-cont  flex-md-fill d-flex flex-column'>
            <div class="raise_voice_banner row align-items-center m-0 mb-3">
                <div class="col-4">
            ...
            <div class="blog-list-section bg-w-block py-4 d-block">
                ...
                if (empty($data)) {
                    ...
                    <div class='row mx-3 my-2 pollcont'>
                        ...
                        ?>
                        <?php if ($data[0]['image'] && !@$data[0]['data']) { ?>
                        ...
                        <div class="col-12">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2 col-3 p-0 px-md-3">
                                    <div class=" position-relative votes-bg position-relative d-flex align-items-center justify-content-center">
                                        <h6 class="m-0 font-weight-500" style=""><?= convert_numbers($data[0]['total_votes']); ?></h6>
                                        <img src="<?= base_url("/images/icons/v2.svg?v=1") ?>" class="position-absolute votsex" style="">
                                    </div>
                                    <h6 class="font-weight-500 w-100 text-center pt-3">Votes</h6>
                                </div>
                                <div class="col-md-10 col-9 mb-3">
                                    <div class="row">
                                        <h1 class="font-weight-300 d-inline col-12 mb-md-2 mb-0 font-weight-175"><?= $title ?></h1>
                                        <?php
        ...
        // echo "</pre>";
                    //   die;                  ?>
                                        <div class="generated-preview mt-3 w-100" style="position:relative;">
                                            <div class="<?= (!@$data[0]['data']) ? "d-none" : "" ?>">
                                                <a href="<?= $preview_data['link'] ?>" target="_blank" class="text-dark nav-link">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-sm-8">
                                                            <div class="contain-title"><?= $preview_data['title']."1st" ?></div>
                                        ...
                                        if ($data[0]['user_id'] === $uid) {
                                            ?>
                                            <h6 id="blogedit" class='comment-action col-12'>
                                                <a href="<?= base_url() ?>Predictions/raise_prediction/<?= $prediction_id ?>" class="a-nostyle edit-voice" data-id="<?= $prediction_id ?>"><small>EDIT</small>
                                                </a>
                                                <span>&nbsp;&nbsp;&#9679;&nbsp;&nbsp;</span>
                                                <a href="#" class="a-nostyle delete-voice" data-id="<?= $prediction_id ?>" data-toggle="modal" data-target="#cnfDeleteVoiceModal"><small>DELETE</small>
                                                </a>
                                            </h6>
                                            <?php
                                        }
                                        ?>
                                        <div class="mb-md-1 col-md-6 mt-mb-3">
                                ...
                                <div class="col-lg-6 offset-lg-6 col-12 share-prop">
                                    <div class="row justify-content-md-end justify-content-center">
                                        <a class="d-flex align-items-center bg-facebook text-white rounded cust-btn btn col   mx-1"
                                           href="http://www.facebook.com/sharer/sharer.php?u=<?= $share_url ?>" target="_blank">
                                            <i class="fa w-50 fa-facebook"></i>
                                            <h6 class="mb-0 w-100">Share</h6>
                                        </a>
                                        <a class="d-flex align-items-center bg-twitter text-white rounded cust-btn btn col  mx-1" 
                                           href="https://twitter.com/intent/tweet?url=<?= $share_url ?>&text='<?= urlencode($title) ?>'&ael;hashtags=Crowdwisdom" target="_blank">
                                            <i class="fa w-50 fa-twitter"></i>
                                            <h6 class="mb-0 w-100">Tweet</h6>
                                        </a>
                                        <a class="d-flex bg-linkdein align-items-center text-white rounded cust-btn btn col  mx-1"
                                           href="https://www.linkedin.com/shareArticle?mini=true&url=<?= $share_url ?>&title=<?= urlencode($title) ?>"  target="_blank">
                                            <i class="fa w-50 fa-linkedin"></i>
                                            <h6 class="mb-0 w-100">Share</h6>
                                        </a>
                                        <a class="d-flex bg-whatsapp align-items-center text-white rounded cust-btn btn col   mx-1"
                                           href="https://wa.me/?text=<?= $share_url ?>"   target="_blank">
                                            <i class="fa w-50 fa-whatsapp"></i>
                                            <h6 class="mb-0 w-100">Share</h6>
                                        </a>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                ...
                                ?>
                                <div class="col-md-4 ml-auto expertsw align-items-center d-flex justify-content-end <?= $hide ?>">
                                    <p class="small d-inline-block mb-2 mr-2">See Expert results</p><span class="d-inline-block">
                                        <a href='#' class='tgl tgl-ios'></a>
                                        <label class="tgl-btn check_expert_result" for="cb2" data-pollid="<?= $did ?>"></label>
                                    </span>

                                </div>
                            </div>

                            <div class='row mb-2 poll-options'>
                                <div class="col-12">
                                    <form class="form">
                                        <?php
                                        foreach ($data[0]['options'] as $option):
                                            ...
                                            $class = ""; //$userchoice == $choiceid ? "bg-active" : "";
                                            $input = $enddate ? "disabled" : "";
                                            $hide = "";

                                            echo '<div class="bg-body-light position-relative w-100 py-2 rounded-semi mb-3 choice-cont ' . $class . '">';
                                            if (!$showper)
                                                $hide = "d-none";
                                            echo '<div class="activity position-absolute h-100 bg-active ' . $hide . '" style="width:' . $option['avg'] . '%"  data-avg="' . $option['avg'] . '"></div>
                                                            <div class="custom-control custom-radio position-relative">
  <input type="radio" id="choiceradio' . $choiceid . '" ' . ( $userchoice == $choiceid ? "checked" : "" ) . ' ' . $input . ' name="choiceradio" value="' . $choiceid . '" class="custom-control-input choose-option">
  <label class="custom-control-label w-100 m-0" for="choiceradio' . $choiceid . '"><span class="font-weight-400 option-name" data-id="' . $choiceid . '">' . $choice . '</span>
          <span class="font-weight-400 float-right avgval ' . $hide . '">' . ((((float) $option['avg']) * 100) / 100) . '%</span></label>
</div>  
                                        </div>';
                                        endforeach;
                                        ?>
                            ...
                            <div class="" id="bloglikecomment">
                                <div class="d-flex align-items-center">
                                    ...
                                    <?php } ?>
                                    <!--<a href="#" data-toggle="modal" data-target="#trendGraph" class="rounded-semi btn btn-default bg-w-block shadow trend">Trend</a>-->
                                    <small class="likes ml-auto"><span class='total-likes-count'><?= "" /* @$data[ 0 ][ 'total_views' ] */ ?> <!--Views &nbsp;&nbsp;&#x25cf;&nbsp;&nbsp; --> <?= @$data[0]['total_comments'] ?> Comments</small>
                                </div>
                            </div>
                            ...
                            if (!empty($ids)) {
                                ...
                                <div class="d-flex justify-content-between mt-5">
                                    ...
                                    <a href="<?= base_url("Packages/detail/" . $id . "/" . $prev); ?>" class="rounded-semi btn btn-primary shadow mr-3 <?= ($prev < 0) ? 'disabled' : '' ?> ">Previous</a>

                                    <a href="<?= base_url("Packages/detail/" . $id . "/" . $next); ?>" class="rounded-semi btn btn-primary shadow mr-3 <?= ($next < 0) ? 'disabled' : '' ?> ">Next</a>
                            ...
                            <div id="comentbox" class=" my-3 py-1">
                                ...
                                <div class="comment_container">
                                    <h4 class='my-3 font-weight-400'><small>Reader's Comments</small><!--<small class="pull-right font-italic">Write a comment and earn Rs. 50!</small>--></h4>
        ...
        <div class="col-md-3 p-md-0 py-4 flex-md-fill d-md-flex flex-column">
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <p>Are you sure want to delete this predection?</p>
            </div>
...
<div class="modal fade" id="trendGraph" role="dialog" tabindex="-1">
...
<script>
    ...
    var votestatus = <?= ($showper) ? 1 : 0 ?>;
    var relatedTopicIds = <?= json_encode($selectedTopicIds) ?>;
    var election_category_exist = '<?= $election_category_exist; ?>';
    var allow_to_for_election = '<?= $allow_to_for_election ?>';
</script>
<script src="<?= base_url() ?>js/polls.js?v=1.54"></script>

<script>
    var config = {
        type: 'line',
        data: {
            labels: ['Week 49', 'Week 50', ],
            datasets: [
                {
                    label: 'See  Results',
                    borderColor: 'red',
        ...
        options: {
            ...
            title: {
                display: true,
                text: 'Jana Sena Vote Share Weekly Trend'
            },
            tooltips: {
                mode: 'index',
                intersect: false,
