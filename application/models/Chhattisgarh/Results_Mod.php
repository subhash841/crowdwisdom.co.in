...
class Results_Mod extends CI_Model {
        ...
        function get_avg_seat_vote_forecast( $election_period_id, $state_id ) {
                ...
                $array_of_ordered_ids = array ( 1, 2, 3, 7, 17, 18 );
                $order = sprintf( 'FIELD(p.id, %s)', implode( ', ', $array_of_ordered_ids ) );

                $this -> db -> select( "uf.id,uf.election_period_id,uf.state_id,uf.party_id,AVG(uf.seat_forecast) as avg_seatforecast,p.name as party_name, p.bilingual_name, p.abbreviation, p.icon" );
