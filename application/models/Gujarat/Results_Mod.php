...
class Results_Mod extends CI_Model {
    ...
    function get_seat_vote_avg_forecasting($election_period_id) {
        ...
        $this->db->where("election_period_id = $election_period_id AND state_id = 1 AND user_id not in (select user_id from user_forecasting WHERE election_period_id = $election_period_id and state_id = 1 group by user_id having sum(seat_forecast) = 0)");
        ...
        $this->db->from("user_forecasting uf");
        $this->db->join("parties p", "uf.party_id = p.id", "INNER");
        $this->db->where("election_period_id = $election_period_id AND state_id = 1 AND user_id not in (select user_id from user_forecasting WHERE election_period_id = $election_period_id and state_id = 1 group by user_id having sum(vote_forcast) = 0)");
        $this->db->group_by("party_id");
        //$this->db->order_by("party_id"); for gujrat
        $this->db->order_by("uf.id");
        ...
        $this->db->where("election_period_id = $election_period_id AND state_id = 1");
        $this->db->group_by("party_id");
        //$this->db->order_by("party_id"); for gujrat
        $this->db->order_by("id");
        ...
        $this->db->where("is_active = '1' AND election_period_id = $election_period_id AND state_id = 1");
    ...
    function get_actual_and_users_seat_vote_forecast($id) {
        ...
        $this->db->where("user_id = $id AND uf.election_period_id = '1' AND uf.state_id = '1'");
        $this->db->group_by('party_id');
        $this->db->order_by('party_id');
        $actual_seat_vote_result = $this->db->get()->result_array();
