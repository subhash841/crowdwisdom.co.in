...
class Survey_Mod extends CI_Model {
    ...
    function get_trending_surveys($user_id, $offset = 0, $limit = 10, $section = "") {
        ...
        $avg_votes = $this->calculateavgvotes($allids);
        ...
        $userchoice = [];
        if ($user_id > 0) {
            $userchoice = $this->get_user_choice($allids, $user_id);
        }

//                echo "<pre>";
//                print_r( json_encode( $trending_result ) );
//                echo "All comments";
//
//                echo "<br><br>";
//                print_r( json_encode( $allcomments ) );
//                echo "<br><br>";
//
//                echo "Comment";
//                echo "<br><br>";
//                print_r( json_encode( $this -> get_user_choice( $allids, $user_id ) ) );

        for ($i = 0; $i < count($trending_result); $i ++) {
            $va = array();
            foreach ($avg_votes as $av) {
                if ($av['poll_id'] == $trending_result[$i]['id']) {
                    $va[] = $av;
                }
            }

            $trending_result[$i]['options'] = $this->getavg($va);

            foreach ($allcomments as $av) {
                if ($av['poll_id'] == $trending_result[$i]['id']) {
                    $trending_result[$i]['All_comments'][] = $av;
                }
            }

            foreach ($userchoice as $av) {
                if ($av['poll_id'] == $trending_result[$i]['id']) {

                    $trending_result[$i]['user_choice'] = $av['choice'];
                }
            }

            foreach ($commentcounts as $av) {
                if ($av['poll_id'] == $trending_result[$i]['id']) {
                    $trending_result[$i]['total_comments'] = $av['count'];
                }
            }

            foreach ($counttoberemoved as $av) {
                if ($av['poll_id'] == $trending_result[$i]['id']) {
                    $trending_result[$i]['total_votes'] = $trending_result[$i]['total_votes'] - $av['count'];
                }
            }


            // $trending_result[ $key ][ 'All_comments' ] = $allcomments
            // $trending_result[ $key ][ 'total_comments' ] = $commentcounts;
...
//                exit();
    ...
    function get_user_choice($id, $user_id) {
        ...
        $this->db->select("choice, poll_id");
        $this->db->from("survey_action");
        $this->db->where_in("poll_id", $id);
        $this->db->where('user_id', $user_id);
        $user_choice_result = $this->db->get()->result_array();
    ...
    function get_my_raised_survey($userid, $offset = 0, $limit = 10) {
        $this->db->select("s.id,s.question,s.total_votes");
        $this->db->from("survey s");
        ...
        $this->db->offset($offset);
        $this->db->limit($limit);
        $my_raised_result = $this->db->get()->result_array();

        foreach ($my_raised_result as $key => $my_raised_data) {
            $id = $my_raised_data['id'];

            $counttoberemoved = $this->remove_votes_count($id)['count'];
            $my_raised_result[$key]['total_votes'] = $my_raised_result[$key]['total_votes'] - $counttoberemoved;
        }
        return $my_raised_result;
    }

    function get_my_answered_surveys($userid, $offset = 0, $limit = 10) {
        $pollids = array();
        $my_answered_result = array();

        if ($userid > 0) {
            $this->db->select("sa.poll_id");
            $this->db->from("survey_action sa");
            $this->db->where("user_id = '$userid'");
            $answered_surveys = $this->db->get()->result_array();

            foreach ($answered_surveys as $data_ids) {
                $pollids[] = $data_ids['poll_id'];
            }
        }
        ...
        if (!empty($pollids)) {
            ...
            $this->db->from("survey s");
            $this->db->where("s.is_active = '1'");
            $this->db->where_in("s.id", $pollids);
            $this->db->order_by("s.id DESC");
            $this->db->offset($offset);
            $this->db->limit($limit);
            $my_answered_result = $this->db->get()->result_array();

            foreach ($my_answered_result as $key => $my_answered_data) {
                $id = $my_answered_data['id'];

                $counttoberemoved = $this->remove_votes_count($id)['count'];
                $my_answered_result[$key]['total_votes'] = $my_answered_result[$key]['total_votes'] - $counttoberemoved;
            }
        } else {
            $my_answered_result = array();
        }
        return $my_answered_result;
    }
...
//                'category_id' => 0,
//                'action' => 'Survey Raised'
//            );
//             $this->db->insert("survey_action", $points);
...
//            $_SESSION['data']['silver_points'] = $sessiondata['silver_points'] - 500;
    ...
    function get_poll_by_id($id, $user_id, $offset = 0, $limit = 10) {
        $user_choice_cond = "";
        if ($user_id != 2) {
            $user_choice_cond = ",(SELECT choice from survey_action  where poll_id = p.id AND  action= 'Vote' AND user_id=" . $user_id . ") as user_choice";
        }
        $this->db->select("p.*,pc.name as category_name,u.alise as byuser,GROUP_CONCAT(pc1.id) as choice_id,"
                . "GROUP_CONCAT(pc1.choice) as choice $user_choice_cond");
        ...
        if ($mydiscussion['id'] != "") {
            ...
            $mydiscussion['options'] = $this->getavg($mydiscussion['options']);
            ...
            $counttoberemoved = $this->remove_votes_count($survey_id)['count'];
...
//                ."(SELECT count(neutral) from survey_action  where poll_id = f.id AND neutral = 1) as total_neutral"
        ...
        $avg_votes = $this->calculateavgvotes($id);


        for ($i = 0; $i < count($mypoll); $i ++) {
            $va = array();
            foreach ($avg_votes as $av) {
                if ($av['poll_id'] == $mypoll[$i]['id']) {
                    $mypoll[$i]['options'] = $av;
                }
            }
        }
    ...
    function get_poll_by_category($category, $user_id, $pollid = 0, $offset = 0, $limit = 10) {
        ...
        $this->db->select("sa.poll_id, count(1) as max_votes");
        $this->db->from("survey_action sa");
        $this->db->where("(now() - interval 4 hour <= (sa.created_date + INTERVAL 30 MINUTE)) AND sa.action= 'Vote' $myvoted_cond");
        $this->db->group_by("sa.poll_id");
        $this->db->order_by("max_votes ASC");
        $this->db->order_by("sa.poll_id ASC");
        $result = $this->db->get();
        ...
        $this->db->select("p.*,pc.name as category_name,u.alise as byuser,GROUP_CONCAT(pc1.id SEPARATOR '|') as choice_id,"
                ...
                . "(CASE WHEN (p.modified_date + INTERVAL 30 MINUTE) >= (now() - interval 1 hour) THEN 1 ELSE 0 END) as is_new,"
        ...
        if ($fourhourvotedids != "") {
            ...
            $fourhourvotedids = $fourhourvotedids . "" . $pollid_also;
            $this->db->order_by("is_new DESC, is_user_voted ASC, FIELD(p.id, $fourhourvotedids) DESC,p.id DESC");
