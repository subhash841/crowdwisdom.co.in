...
class Results_Mod extends CI_Model {
    ...
    function get_avg_sport_forecast($sport_period_id, $sport_id) {
        /* Get AVG Score Forecasts */
        $this->db->select("sf.id,sf.sport_period_id,sf.sport_id,sf.team_id,AVG(sf.score_forecast) as avg_scoreforecast,st.name as team_name, st.abbreviation, st.icon");
        $this->db->from("sport_forecasting sf");
        $this->db->join("sport_teams st", "sf.team_id = st.id", "INNER");
        $this->db->where("sport_period_id = '$sport_period_id' AND sf.sport_id = '$sport_id' AND user_id not in (select user_id from sport_forecasting WHERE sport_period_id = '$sport_period_id' and sport_id = '$sport_id' group by user_id having sum(score_forecast) = 0)");
        $this->db->group_by("team_id");
        $this->db->order_by("sf.id");
        $avg_forecasting['avg_scoreforecast'] = $this->db->get()->result_array();

        /* Get AVG Wicket Forecasts */
        $this->db->select("sf.id,sf.sport_period_id,sf.sport_id,sf.team_id,AVG(sf.wicket_forecast) as avg_wicketforcast,st.name as team_name, st.abbreviation, st.icon");
        $this->db->from("sport_forecasting sf");
        $this->db->join("sport_teams st", "sf.team_id = st.id", "INNER");
        $this->db->where("sport_period_id = '$sport_period_id' AND sf.sport_id = '$sport_id' AND user_id not in (select user_id from sport_forecasting WHERE sport_period_id = '$sport_period_id' and sport_id = '$sport_id' group by user_id having sum(wicket_forecast) = 0)");
        $this->db->group_by("team_id");
        $this->db->order_by("sf.id");
        $avg_forecasting['avg_wicketforcast'] = $this->db->get()->result_array();

        //Get actual result for SEAT and VOTE forecast
        $this->db->select("sport_period_id,sport_id,team_id,actual_score,actual_wicket");
        $this->db->from("matches");
        $this->db->where("is_active = '1' AND sport_period_id = '$sport_period_id' AND sport_id = '$sport_id'");
        $this->db->order_by('team_id');
        $actual_seat_vote_result = $this->db->get()->result_array();

        $forecast = [];
        foreach ($avg_forecasting['avg_scoreforecast'] as $key => $forecasting) {

            $avg_forecasting['avg_scoreforecast'][$key]['avg_wicketforcast'] = 0;
            if (isset($avg_forecasting['avg_wicketforcast'][$key])) {
                $avg_forecasting['avg_scoreforecast'][$key]['avg_wicketforcast'] = $avg_forecasting['avg_wicketforcast'][$key]['avg_wicketforcast'];
            }

            $avg_forecasting['avg_scoreforecast'][$key]['actual_score'] = $actual_seat_vote_result[$key]['actual_score'];
            $avg_forecasting['avg_scoreforecast'][$key]['actual_wicket'] = $actual_seat_vote_result[$key]['actual_wicket'];

            $forecast[] = $avg_forecasting['avg_scoreforecast'][$key];
        }
