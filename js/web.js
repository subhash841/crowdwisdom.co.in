...
var textarea = document.querySelector('.cust-textarea');
   ...
   function autosize() {
    ...
    $("a.head-login").attr("href", base_url + "Login?section=web");

    /* This is for detail view */
    if (id != "") {
        $("a.head-login").attr("href", base_url + "Login?section=webdetail&vid=" + id);
    }


    /* Points checking for create voice */
    $(".btn_create_web").on("click", function (e) {
        e.preventDefault();
        ...
        if (uid == 0) {
            if (id == "") {
                window.location.assign(base_url + "Login?section=web");
            } else {
                window.location.assign(base_url + "Login?section=webdetail&vid=" + id);
            }
            ...
            body = '<div><img src="' + base_url + 'images/banners/coin_f.png"><span>' + silver_points + '</span></div><p>Redeem 25 Points to start discussion.</p>';
            $("#silverPointsCheck .modal-title").html("Your Balance Points");
            $('#silverPointsCheck .modal-body').html(body);
    ...
    $('#silverPointsCheck button#create_yes').on("click", function () {
        window.location.assign(base_url + "FromTheWeb/post_article");
    });
    ...
    $("#cnfDeleteModal button#btn_delete_yes").on("click", function (event) {
        ...
        $.ajax({
            url: base_url + "FromTheWeb/deactive_web",
            method: "POST",
            data: {id: id}
    ...
    $(".post-comment").on("click", function (e) {
        ...
        $.ajax({
            url: base_url + "FromTheWeb/add_comment",
            method: "POST",
            data: {id: wid, comment: comment}
        }).done(function (result) {
            if (result.status) {
    ...
    $(document).on("click", ".post-edit-comment", function (e) {
        ...
        _this.addClass("disabled");
        $.ajax({
            url: base_url + "FromTheWeb/update_comment",
            method: "POST",
    ...
    $(document).on("click", ".show-hide-reply", function (e) {
        ...
        var id = _this.attr("data-id");
        if (uid == 0) {
            window.location.assign(base_url + "Login?section=walldetail&vid=" + id);
            return false;
        }
        var comment_id = _this.attr("data-comment_id");
        var offset = 0;
        ...
        $.ajax({
            url: base_url + "FromTheWeb/get_comment_replies",
            method: "POST",
    ...
    $(document).on("click", ".post-comment-reply", function (e) {
        ...
        $.ajax({
            url: base_url + "FromTheWeb/add_comment_reply",
            method: "POST",
    ...
    $(document).on("click", ".post-edit-comment-reply", function (e) {
        ...
        _this.addClass("disabled");
        $.ajax({
            url: base_url + "FromTheWeb/update_comment_reply",
            method: "POST",
    ...
    $(".rounded-circle ").hover(function () {
        $(this).children('img').css({filter: 'grayscale(1) brightness(100)'});
    }, function () {
        if ($(this).parent(".wall-action").attr('data-val') != '1') {
            $(this).children('img').css({filter: ''});
        }
    });

    $('.wall-action').each(function (key, value) {
        if ($(this).attr('data-val') == '1') {

            if ($(this).children().first().hasClass('btn-outline-success')) {
                $(this).children().first().removeClass('btn-outline-success').addClass('btn-success');
                $(this).find('img').css({filter: 'grayscale(1) brightness(100)'});
            }
            if ($(this).children().first().hasClass('btn-outline-danger')) {
                $(this).children().first().removeClass('btn-outline-danger').addClass('btn-danger');
                $(this).find('img').css({filter: 'grayscale(1) brightness(100)'});
            }
            if ($(this).children().first().hasClass('btn-outline-info')) {
                $(this).children().first().removeClass('btn-outline-info').addClass('btn-info');
                $(this).find('img').css({filter: 'grayscale(1) brightness(100)'});
            }

        }
    });
    $(".wall-action").on("click", function (e) {
        e.preventDefault();
        if (uid == 0) {
            localStorage.id = id;
            //localStorage.choice = choice;
        ...
        //var actionValue = $(this).attr("data-val");
        $.ajax({
            url: base_url + "FromTheWeb/add_update_action",
            method: "POST",
            data: {id: id, type: type}
        }).done(function (result) {
            /*setTimeout(function () {
             _this.removeAttr("disabled").html('Yes');
             window.location.assign(base_url + "Wall");
             }, 2000);*/
...
function counter(el, a) {
    ...
    if (a != el.text()) {
        a = a.replace("%", "")
        el.prop('Counter', el.text().replace("%", "")).animate({
            Counter: a
        }, {
            duration: 400,
            easing: 'swing',
...
var callback_delete_comment = function (e) {
    ...
    var total_comments = parseInt($("#comentbox .total-comments-count").html());
    $.ajax({
        url: base_url + "FromTheWeb/delete_comment",
        method: "POST",
...
var callback_delete_comment_reply = function (e) {
    ...
    var comment_reply_id = $(this).attr("data-reply_id");
    $.ajax({
        url: base_url + "Wall/delete_comment_reply",
        method: "POST",
...
var view_more_comments = function (e) {
    ...
    var comment_html = '';
    $.ajax({
        url: base_url + "Wall/view_more_comments",
        method: "POST",
...
function getblock(data, q) {
    ...
    if (q == "AskQuestions/details/" || q == "Predictions/details/" || q == "FromTheWeb/detail/") {
        ...
        likes = $("<small />", {class: "likes"}).html(total_votes + " Likes &nbsp;&nbsp;&#9679;&nbsp;&nbsp; " + total_views + " Views &nbsp;&nbsp;&#9679;&nbsp;&nbsp; " + total_comments + " Comments");
    }
