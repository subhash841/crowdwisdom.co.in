   ...
   function autosize() {
    ...
    if (localStorage.id)
    {
        if (localStorage.id == id) {
            ...
            choice = JSON.parse(localStorage.choice);
    ...
    if (id != "") {
        $("a.head-login").attr("href", base_url + "Login?section=questiondetail&vid=" + id);
    }

    $(".choose-option").on("change", function (e) {
        var _this = $(this);
        if (_this.closest(".poll-options").hasClass("single-choice"))
            $('.choice-cont').removeClass('bg-active');
        if (_this.prop("checked"))
            _this.closest('.choice-cont').addClass('bg-active');
        else
            _this.closest('.choice-cont').removeClass('bg-active');
    })
    ...
    $(".showoptions").on("click", function (e) {
        ...
        $(".choice-cont.d-none").removeClass("d-none").slideDown(1000)
    })
    $(".vote").on('click', function (e) {
        ...
        var choice = [];
        $("input.choose-option:checked", p).each(function () {
            choice.push($(this).val())
        });
        if (uid == 0) {
            localStorage.id = id;
            localStorage.choice = JSON.stringify(choice);
            window.location.assign(base_url + "Login?section=questiondetail&vid=" + id);
            return false;
        }
        $.post(base_url + "AskQuestions/vote_action", {id: id, choice: choice}).done(function (d) {
...
//                obj = [];
                ...
                $(".choice-cont input").attr("disabled", "disabled")
    ...
    $(".post-comment").on("click", function (e) {
        ...
        $.ajax({
            url: base_url + "AskQuestions/add_comment",
            method: "POST",
    ...
    $(document).on("click", ".post-edit-comment", function (e) {
        ...
        _this.addClass("disabled");
        $.ajax({
            url: base_url + "AskQuestions/update_comment",
            method: "POST",
    ...
    $(document).on("click", ".show-hide-reply", function (e) {
        ...
        $.ajax({
            url: base_url + "AskQuestions/get_comment_replies",
            method: "POST",
    ...
    $(document).on("click", ".post-comment-reply", function (e) {
        ...
        $.ajax({
            url: base_url + "AskQuestions/add_comment_reply",
            method: "POST",
    ...
    $(document).on("click", ".post-edit-comment-reply", function (e) {
        ...
        _this.addClass("disabled");
        $.ajax({
            url: base_url + "AskQuestions/update_comment_reply",
            method: "POST",
    ...
    $("#cnfDeleteVoiceModal button#btn_delete_yes").on("click", function (event) {
        var _this = $(this);
        var id = $("#cnfDeleteVoiceModal #del_id").val();

        _this.attr("disabled", "disabled").html('Processing...');
        $.ajax({
            url: base_url + "Predictions/delete_voice",
            method: "POST",
            data: {id: id}
        }).done(function (result) {
            setTimeout(function () {
    ...
    $(".btn_create_question").on("click", function (e) {
        ...
        if (uid == 0) {
            window.location.assign(base_url + "Login?section=createquestion");
        } else {
            window.location.assign(base_url + "AskQuestions/raise_question");
        }
//        if (uid == 0) {
//            if (id == "") {
//                window.location.assign(base_url + "Login?section=question");
//            } else {
//                window.location.assign(base_url + "Login?section=questiondetail&vid=" + id);
//            }
...
var callback_delete_comment = function (e) {
    ...
    var total_comments = parseInt($("#comentbox .total-comments-count").html());
    $.ajax({
        url: base_url + "AskQuestions/delete_comment",
        method: "POST",
...
var callback_delete_comment_reply = function (e) {
    ...
    var comment_reply_id = $(this).attr("data-reply_id");
    $.ajax({
        url: base_url + "AskQuestions/delete_comment_reply",
        method: "POST",
...
var view_more_comments = function (e) {
    ...
    var comment_html = '';
    $.ajax({
        url: base_url + "AskQuestions/view_more_comments",
        method: "POST",
...
function getblock(data, top = false) {
    ...
    var a = $("<a/>", {
        ...
        class: "d-block pb-3",
        "data-content": "Most Popular Question",
