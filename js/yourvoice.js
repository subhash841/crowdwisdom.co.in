...
$(function () {
   ...
   function autosize() {
    ...
    $("a.head-login").attr("href", base_url + "Login?section=voice");
    var default_voice_count = 7;
    ...
    $(".blog-list-page .load-more-voice-list").on("click", function (e) {
        if (voice_list_offset == 0) {
            default_voice_count = 7;
        } else {
            default_voice_count = 6;
        }
        voice_list_offset = voice_list_offset + default_voice_count;
        load_voices(voice_list_offset);
    });
    /* Points checking for create voice */
    $(".btn_create_voice").on("click", function (e) {
        ...
        var body = '';
        if (uid == 0) {
            window.location.assign(base_url + "Login?section=voice");
    ...
    $('#silverPointsCheck button#create_voice_yes').on("click", function () {
        window.location.assign(base_url + "YourVoice/raise_voice");
    });
    if (options.toast) {
        return false;
    }
...
//
//function getblock(data, q = "predictions") {
//
//    if (q == "voices")
//        q = "YourVoice/blog_detail/"
//    else if (q == "questions")
...
//    var a = $("<a/>", {
//        href: base_url + 'YourVoice/blog_detail/' + data.id + '/' + slug,
//        class: "d-block pb-3",
//        "data-content": "Most Popular Blog",
